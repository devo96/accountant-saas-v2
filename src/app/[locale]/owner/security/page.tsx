...
export default async function OwnerSecurityPage() {
  ...
  const [totalOrgs, totalUsers, activeOrgs, totalRevenue, auditLast7d, auditToday, totalJournalEntries, totalInvoices] = await Promise.all([
    prisma.organization.count(), prisma.user.count(), prisma.organizationPlan.count({ where: { status: "ACTIVE" } }), prisma.salesInvoice.aggregate({ _sum: { total: true } }), prisma.auditLog.count({ where: { createdAt: { gte: sevenDaysAgo } } }), prisma.auditLog.count({ where: { createdAt: { gte: todayStart } } }), prisma.journalEntry.count(), prisma.salesInvoice.count(),
  ]);
  ...
  return (
    <FadeIn>
      <div className="space-y-4">
        <PageHeader title={t("securityTitle")} description={t("securitySubtitle")} />
