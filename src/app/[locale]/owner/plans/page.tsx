...
export default async function OwnerPlansPage() {
  ...
  const plansWithCount = plans.map((p) => ({ id: p.id, name: p.name, tier: p.tier, monthlyPrice: Number(p.monthlyPrice), yearlyPrice: Number(p.yearlyPrice), maxUsers: p.maxUsers, maxInvoices: p.maxInvoices, active: p.active, orgCount: countMap.get(p.id) ?? 0 }));
  return (
    <FadeIn>
      <div className="space-y-4">
        <PageHeader title={t("plansTitle")} description={t("plansSubtitle")} />
