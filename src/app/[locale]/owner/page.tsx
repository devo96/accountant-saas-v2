...
export default async function OwnerDashboardPage() {
  ...
  const [totalOrgs, totalUsers, orgPlans, invoices, orgs, usersByOrg, todayJournals, todayInvoices] = await Promise.all([
    prisma.organization.count(), prisma.user.count(), prisma.organizationPlan.groupBy({ by: ["status"], _count: true }), prisma.salesInvoice.findMany({ select: { total: true, invoiceDate: true } }), prisma.organization.findMany({ orderBy: { createdAt: "desc" }, include: { organizationPlan: { include: { plan: true } }, _count: { select: { users: true } } } }), prisma.user.groupBy({ by: ["organizationId"], _count: true }), prisma.journalEntry.count({ where: { createdAt: { gte: todayStart } } }), prisma.salesInvoice.count({ where: { createdAt: { gte: todayStart } } }),
  ]);
  ...
  const chartData = months.map((month) => ({ month, revenue: Math.round(monthlyRevenue[month]) }));
  ...
  return (
    <FadeIn>
      <div className="space-y-4">
        <PageHeader title={t("title")} description={t("overviewSubtitle")} />
        <OwnerOverviewClient
          totalOrgs={totalOrgs} totalUsers={totalUsers} mrr={mrr} arr={arr}
          churnRate={churnRate} liveOps={{ journals: todayJournals, invoices: todayInvoices }}
          activeOrgs={Number(activeOrgCount)} trialingOrgs={Number(trialingOrgCount)}
          expiredOrgs={Number(expiredOrgCount)} cancelledOrgs={Number(cancelledOrgCount)}
          orgsThisMonth={orgsThisMonth} newUsersThisMonth={newUsersThisMonth} unassignedOrgs={unassignedOrgs}
          chartData={chartData}
        />
      </div>
    </FadeIn>
