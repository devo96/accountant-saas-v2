...
export default async function CustomerReceiptsPage() {
  ...
  const receipts = await prisma.paymentReceipt.findMany({
    where: { organizationId: session.user.organizationId, salesInvoiceId: { not: null } },
    orderBy: { date: "desc" },
        ...
        <Table>
          ...
          <TableBody>
            {receipts.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="py-12 text-center text-gray-500">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span>{tp("noReceipts") ?? tc("noData")}</span>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              receipts.map((r) => (
                <TableRow key={r.id}>
                  <TableCell className="font-medium">{generateNumber("REC", r.number)}</TableCell>
                  <TableCell>{r.salesInvoice?.customer?.name ?? "—"}</TableCell>
