...
type Customer = { id: string; name: string; taxNumber?: string | null };
...
type Props = { customers: Customer[]; items: Item[]; taxCodes: TaxCode[]; paymentTerms: PaymentTerm[]; branches: Branch[]; projects: Project[] };
...
export function NewInvoiceClient({ customers: initialCustomers, items: initialItems, taxCodes, paymentTerms, branches, projects: initialProjects }: Props) {
  ...
  const handleCustomerCreated = (entity: { id: string; name: string }) => {
    setCustomers([...customers, { id: entity.id, name: entity.name }]);
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit("CONFIRMED"); }} className="space-y-5">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button
              type="button"
              variant="outline"
              disabled={submitting || !customerId || lines.length === 0 || lines.some((l) => !l.itemId)}
