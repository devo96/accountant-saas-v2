...
export default async function OwnerPaymentsPage() {
  ...
  const data = payments.map((p) => ({
    id: p.id, organizationId: p.organizationId, planId: p.planId,
    amount: Number(p.amount), currency: p.currency, type: p.type, status: p.status,
    paymentMethod: p.paymentMethod, receiptUrl: p.receiptUrl, reason: p.reason,
  ...
  return (
    <FadeIn>
      <div className="space-y-4">
        <PageHeader title="Payments" description="Payment and renewal history" />
