...
export function PlansClient({ plans: initialPlans }: { plans: PlanExt[] }) {
  ...
  async function deletePlan(id: string) {
    ...
    await fetch(`/api/owner/plans/${id}`, { method: "DELETE" });
    refresh();
  }
          ...
          return (<Card key={plan.id} className={plan.active ? "" : "opacity-50"}>
            ...
            <CardContent className="space-y-2">
              ...
              <div className="space-y-1 text-xs">
                ...
                <p className="flex justify-between"><span className="text-gray-400">Organizations</span><span>{plan.orgCount}</span></p>
              </div>
