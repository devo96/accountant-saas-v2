...
import { NewTicketForm } from "../_forms";
...
export function SupportClient({ tickets: initialTickets }: { tickets: TicketInfo[] }) {
  ...
  async function updateStatus(id: string, status: string) {
    await fetch(`/api/owner/tickets/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) });
    refresh();
  }

  return (
    <div>
      <div className="flex justify-end mb-3"><Button size="sm" onClick={() => setTicketDialog(true)}><Plus className="h-4 w-4 mr-1" />New Ticket</Button></div>
      <Card>
        <CardHeader><CardTitle className="text-sm"><Ticket className="h-4 w-4 inline-block mr-1" />Support Tickets ({tickets.length})</CardTitle></CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>{tickets.length === 0 ? <tr><td colSpan={5} className="p-6 text-center text-gray-400">No tickets</td></tr> : tickets.map((t) => (
                <tr key={t.id} className="border-b hover:bg-gray-50">
                  <td className="p-3 font-medium">{t.subject}</td>
                  <td className="p-3 text-gray-500">{t.organization.name}</td>
                  <td className="p-3"><span className={`font-medium ${priorityColor[t.priority] ?? ""}`}>{t.priority}</span></td>
                  <td className="p-3">
                    <Select options={[{ value: "OPEN", label: "Open" }, { value: "IN_PROGRESS", label: "In Progress" }, { value: "RESOLVED", label: "Resolved" }, { value: "CLOSED", label: "Closed" }]} value={t.status} onChange={(e) => updateStatus(t.id, e.target.value)} />
                  </td>
