...
import { TrendingUp, Calendar, Percent, Activity, BarChart3 } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";

type ChartData = { month: string; revenue: number };

export function OwnerOverviewClient({
  totalOrgs, totalUsers, mrr, arr, churnRate, liveOps,
  activeOrgs, trialingOrgs, expiredOrgs, cancelledOrgs,
  orgsThisMonth, newUsersThisMonth, unassignedOrgs, chartData,
}: {
  totalOrgs: number; totalUsers: number; mrr: number; arr: number;
  churnRate: string; liveOps: { journals: number; invoices: number };
  activeOrgs: number; trialingOrgs: number; expiredOrgs: number; cancelledOrgs: number;
  orgsThisMonth: number; newUsersThisMonth: number; unassignedOrgs: number;
  chartData: ChartData[];
}) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        <Card><CardContent className="p-3 flex items-center justify-between"><span className="text-xs text-gray-500">Total Orgs</span><span className="text-sm font-bold">{totalOrgs}</span></CardContent></Card>
        <Card><CardContent className="p-3 flex items-center justify-between"><span className="text-xs text-gray-500">Unassigned</span><span className="text-sm font-bold text-amber-600">{unassignedOrgs}</span></CardContent></Card>
      </div>
      ...
      <Card>
        <CardHeader><CardTitle className="text-sm"><BarChart3 className="h-4 w-4 inline-block mr-1" />Monthly Revenue</CardTitle></CardHeader>
        <CardContent>
          <div className="h-48">
