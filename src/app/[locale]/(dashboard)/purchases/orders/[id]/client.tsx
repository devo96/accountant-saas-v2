...
export function PurchaseOrderViewClient({ order: initial }: Props) {
  const t = useTranslations("purchaseOrders");
  const s = useTranslations("common");
  const router = useRouter();
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ status: initial.status, notes: initial.notes ?? "", expectedDate: initial.expectedDate ? new Date(initial.expectedDate).toISOString().split("T")[0] : "" });
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const res = await fetch(`/api/purchase-orders/${initial.id}`, {
        method: "PUT",
    ...
    return (
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" onClick={() => router.push("/purchases/orders")}><ArrowLeft className="h-5 w-5 rtl:scale-x-[-1]" /></Button>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("editOrder")} {generateNumber("PO", initial.number)}</h2>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("orderInfo")} {generateNumber("PO", initial.number)}</h2>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="rounded-lg border p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">{t("taxAmount")}</span><span>{formatCurrency(initial.taxAmount)}</span></div>
