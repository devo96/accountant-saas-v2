...
type Invoice = {
  ...
  notes: string | null; vendor: { id: string; name: string; nameAr?: string | null; taxNumber?: string | null } | null;
...
type Props = { invoice: Invoice; vendors: Vendor[]; items: Item[]; taxCodes: TaxCode[] };
...
export function PurchaseInvoiceViewClient({ invoice: initial, vendors, items, taxCodes }: Props) {
  const router = useRouter();
  const t = useTranslations("purchaseInvoiceView");
  ...
  function updateLine(key: number, field: string, value: string | number) {
    setLines(lines.map((l) => {
      ...
      if (field === "itemId") { const item = items.find((i) => i.id === value); if (item) { updated.description = item.name; updated.unitPrice = item.costPrice; } }
      if (field === "taxCodeId") { const tc = taxCodes.find((t) => t.id === value); if (tc) updated.taxRate = tc.rate; }
      return updated;
    }));
  }
    ...
    try {
      const res = await fetch(`/api/purchase-invoices/${inv.id}`, {
        ...
        body: JSON.stringify({
          ...
          vendorId, notes,
          subtotal: calcSubtotal, discountAmount: calcDiscount, taxAmount: calcTax, total: calcTotal,
          lines: lines.map((l) => ({
            itemId: l.itemId || null, description: l.description, quantity: l.quantity, unitPrice: l.unitPrice,
            discountPercent: l.discountPercent, taxCodeId: l.taxCodeId || null, taxRate: l.taxRate,
  ...
  function cancelEdit() {
    setInv(initial); setVendorId(initial.vendor?.id ?? ""); setDate(toDateStr(initial.invoiceDate));
    setDueDate(toDateStr(initial.dueDate)); setNotes(initial.notes ?? "");
    setLines(initial.lines.map((l) => ({ ...l, key: nextKey++ }))); setEditing(false);
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("title")} PINV-{String(inv.number).padStart(5, "0")}</h2>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="rounded-lg border p-4">
          <h3 className="font-medium text-gray-700 dark:text-gray-300 mb-2">{t("vendorInfo")}</h3>
          <p className="text-lg font-semibold">{inv.vendor?.name ?? "-"}</p>
          {inv.vendor?.taxNumber && <p className="text-sm text-gray-500 dark:text-gray-400">{t("taxNumber")}: {inv.vendor.taxNumber}</p>}
        </div>
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow className="font-bold border-t-2">
              <TableCell colSpan={4} /><TableCell colSpan={2} className="text-right">{t("subtotal")}</TableCell><TableCell className="text-right">﷼ {inv.subtotal.toFixed(2)}</TableCell>
            </TableRow>
